namespace Chess {
	...
	public class MoveGenerator {
		...
		/// </summary>
		public List<Move> GenerateMoves_DO_NOT_USE (Board board, bool includeQuietMoves = true) {
			this.board = board;
		...
		public List<Move> GenerateMoves(Board board, bool isRoot, bool includeQuietMoves = true)
		{
			if (isRoot)
            {
				return GenerateMoves_DO_NOT_USE(board, includeQuietMoves);
            }
			...
			CheckForCheckInMoves();

			return moves;
		}

		void CheckForCheckInMoves()
        {
			int kingSquare = board.KingSquare[1 - board.ColourToMoveIndex];

			for (int i = 0; i < moves.Count; ++i)
            {
				if (moves[i].TargetSquare == kingSquare)
                {
					moves = new List<Move> { moves[i] };
					break;
                }
            }
        }
		...
		public List<SimMove> GetPossibleSimMoves(SimPiece[,] state, bool team)
		{
			...
			for (int i = 0; i < 8; ++i)
			{
				for (int j = 0; j < 8; ++j)
				{
					if (state[i, j] != null && state[i, j].team == team)
					{
						List<SimMove> newMoves = state[i, j].GetMoves(state, i, j);

						// If there is only one new move which finishes the game, only that move is returned.
						// This ensures that the algorithm always takes the enemy's king if it is possible.
						if (
							newMoves.Count == 1 &&
							state[newMoves[0].endCoord1, newMoves[0].endCoord2] != null &&
							state[newMoves[0].endCoord1, newMoves[0].endCoord2].type == SimPieceType.King
							)
						{
							return newMoves;
						}

						result.AddRange(newMoves);
					}
				}
			}
		...
		/// <summary>
		/// Determines whether the current board describes a state in check.
		/// Note, this will only return correct value after GenerateMoves()
		/// has been called in the current position
		/// </summary>
		/// <returns></returns>
		public bool InCheck () {
			return inCheck;
		...
		void GeneratePawnMoves () {
			...
			for (int i = 0; i < myPawns.Count; i++) {
				...
				if (genQuiets) {
					...
					//if (squareOneForward < 0 || squareOneForward > 63)
					//{
					//	continue;
					//}
